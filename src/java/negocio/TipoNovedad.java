...
public class TipoNovedad{
	...
	ArrayList<String> listEstado=new ArrayList<String>();
        ...
        public void getIdTipoNovedadN(String nombre){
            try{
                c = new Coneccion();
                c.setConsulta("select id_tipo_novedad from tipo_novedad where nombre='"+nombre+"'");
                while(c.getRs().next()){
                    
                    listId.add(c.getRs().getString("id_tipo_novedad"));
                }
            }
            catch(Exception e){
